...
def render_experience(items):
    ...
    out=[]
    for j in items:
        out.append(f"<div><strong>{safe_get(j,'title')}</strong> — {safe_get(j,'company')} ({safe_get(j,'location')})<br>{safe_get(j,'start')}–{safe_get(j,'end')}{as_bullets(j.get('highlights',[]))}</div>")
    return "".join(out)
...
def render_education(items):
    ...
    out=[]
    for ed in items:
        out.append(f"<div><strong>{safe_get(ed,'qualification')}</strong> — {safe_get(ed,'institution')} ({safe_get(ed,'start')}–{safe_get(ed,'end')})<br>{safe_get(ed,'details')}</div>")
    return "".join(out)
...
def generate_pdf_download():
    ...
    template_choice = data.get("template","classic").lower()
    template_file = APP_DIR / f"cv_{template_choice}.html"
    if not template_file.exists():
        template_file = APP_DIR / "cv_classic.html"
